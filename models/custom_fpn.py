...
class BottleneckFPNBlock(nn.Module):
    """
    Compress (ConvBNReLU) -> Depthwise (Conv+BN) -> Expand (Conv+BN)
    Prunable ở compress_layer (ConvBNReLU có s_mask/u_mask).
    """

    def __init__(self, channels=256, compress_rate=0.0):
        super().__init__()

        # IMPORTANT: clamp để không bao giờ inner_channels = 0
        inner_channels = max(1, int(round(channels * (1.0 - float(compress_rate)))))
...
class PrunableFPN(nn.Module):
    def __init__(self, in_channels_list, out_channels=256, compress_rate=None):
        ...
        if not isinstance(compress_rate, (list, tuple)):
            # an toàn nếu ai đó truyền scalar
            compress_rate = [float(compress_rate)] * len(in_channels_list)
        ...
        for i, in_c in enumerate(in_channels_list):
            ...
            rate = float(compress_rate[i]) if i < len(compress_rate) else 0.0
    ...
    def forward(self, x):
        # x: Dict hoặc List features từ backbone
